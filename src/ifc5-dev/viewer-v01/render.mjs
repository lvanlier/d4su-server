...
// d4su - Added consoleLog to activate/deactivate detail console.log message
...
function compose(datas) {
    ...
    while (maxIterations --) {
        ...
        if (consoleLog) console.log('Bottom rank prims to resolve:', ...bottomRankNodes);
        ...
        bottomRankNodes.forEach(k => {
            (Array.from(compositionEdgesOrig[k] || [])).forEach(v => {
                ...
                if (consoleLog) console.log('Processing edge:', k, ' --- ', v);
        ...
        if (consoleLog) console.log('Constructed prims:', ...definedPrims);
    ...
    if (consoleLog) console.log(composed['']);

    // d4su - Upload the composed Tree to the FastApi server
    // CORS handling is done in the FastApi server in the main.py file
    const formData = new FormData();
    const obj = composed[''];
    const blob = new Blob([JSON.stringify(obj, null, 2)], {
        type: "application/json",
      });    
    const formData_name = 'file'
    const formData_value = blob
    const form_data_fileName = fileName+'-tree.json'
    formData.append(formData_name, formData_value,form_data_fileName );
    (async () => {
        const rawResponse = await fetch('http://localhost:8000/admin/uploadfile/', {
          method: 'POST',    
          body: formData
        });
...
// d4su - Added the global fileName for the tree ouput file
let fileName = ''
export default function addModel(name, m) {
    // d4su - update the global fileName
