...
class Patcher:
    ...
    def patch(self):
        ...
        fileName = self.src.split("/")[-1].split(".")[0] # LVL added fileName
        storeys = self.file.by_type("IfcBuildingStorey")
        for i, storey in enumerate(storeys):
            storey.Name = storey.Name.replace('/', '-') # LVL replace / with - to avoid path issues
            dest = (
                "{}-{}-{}.ifc".format(fileName, i, storey.Name) # LVL added fileName
                if self.output_dir == None
                else "{}/{}-{}-{}.ifc".format(self.output_dir, fileName, i, storey.Name) # LVL added self.src = fileName
            )
